        ...
        _blocker = {
            ...
            set: function () {
                for (var i = 0; i < arguments.length; i += 1) {
                    if (this.blocked.indexOf(arguments[i]) === -1) {
                        this.blocked.push(arguments[i]);
                    }
                }
            },
            get: function (device) {
                return this.blocked.indexOf(device) !== -1;
            },
            startReset: function () {
                window.clearTimeout(this.timer);
                this.timer = window.setTimeout(this.boundReset, 300);
            },
            reset: function () {
                window.clearTimeout(this.timer);
        ...
        switch (event.phase) {
            ...
            case 'start':
                if (event.device === 'touch') {
                    _blocker.set('mouse');
                    trigger = !_blocker.get('touch');
                } else {
                    trigger = !_blocker.get('mouse');
                }
                if (touchLength > 1) {
            ...
            case 'move':
                if (event.device === 'touch') {
                    ...
                    trigger = !_blocker.get('mouse') && !_blockMove;
                }
                break;
            case 'end':
                if (event.device === 'touch') {
