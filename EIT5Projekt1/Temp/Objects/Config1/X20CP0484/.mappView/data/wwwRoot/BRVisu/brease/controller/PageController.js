...
define(['brease/controller/libs/Client', 'brease/events/BreaseEvent', 'brease/enum/Enum', 'brease/core/Utils', 'brease/controller/libs/LoaderPool', 'brease/controller/libs/PageLogger', 'brease/model/VisuModel', 'brease/controller/libs/LoadCycle', 'brease/controller/libs/ScrollManager', 'brease/controller/libs/Areas', 'brease/controller/libs/Containers', 'brease/helper/Scroller', 'brease/core/libs/Deferred', 'brease/controller/libs/PageTypes', 'brease/controller/libs/AssignTypes', 'brease/controller/libs/Themes', 'brease/controller/libs/VisuStatus', 'brease/controller/libs/LogCode'], function (Client, BreaseEvent, Enum, Utils, LoaderPool, PageLogger, VisuModel, LoadCycle, ScrollManager, Areas, containers, Scroller, Deferred, PageTypes, AssignTypes, Themes, VisuStatus, LogCode) {
    /*jshint white:false */
    'use strict';
    ...
    */
    var PageController = {
        ...
        start: function start(visuId, rootContainer, cachingConfig) {
            //console.log('PageController.start:', visuId, rootContainer.id, cachingConfig);
    ...
    function _activateEmbeddedVisuFailed(visuStatus, code, callbackInfo) {
        if (visuStatus === VisuStatus.ACTIVATE_FAILED) {
            ...
            for (var i = 0; i < contentsToLoad.length; i += 1) {
                _loadCycle.remove(contentsToLoad[i]);
            }
        }
        var data = {
    ...
    function _emptyContainer(container) {
        ...
        if (container.childNodes.length > 0) {
            ...
            for (i = 0, l = collection.length; i < l; i += 1) {
                if (_loaderPool.count <= _loaderPool.maxSlots) {
