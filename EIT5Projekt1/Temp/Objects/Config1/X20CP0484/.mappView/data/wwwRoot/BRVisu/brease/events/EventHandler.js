...
define(['brease/events/BreaseEvent'], function (BreaseEvent) {
    ...
    *
	*         var ev = new EventHandler('widgets.brease.Button.Event', 'button01' , 'Click', eventArgs, elem, 'buttonContent');
    *
    ...
    * @param {String} contentId required for widget events
	*/
	...
	*/
    /**
	* @property {String} contentId (optional)
	*/
	EventHandler = function (eventType, refId, eventName, eventArgs, elem, contentId) {
   ...
   */
    p.dispatch = function (useDom) {
        ...
        if (this.isSubscribed) {
            _runtimeService.dispatchEvent(this.data);
        }
    ...
    function _isSubscribed(refId, eventType, eventName) {
        ...
        if (eventType.indexOf("widgets.") === 0) {
            ...
            events = _bindingController.getEventsForSession(eventName);
            if (events) {
                result = true;
            }
        }
