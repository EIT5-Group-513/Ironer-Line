...
define(['brease/events/BreaseEvent', 'brease/core/Utils', 'brease/controller/libs/PageTypes', 'brease/controller/libs/AssignTypes', 'brease/controller/libs/VisuStatus', 'brease/controller/libs/LogCode'], function (BreaseEvent, Utils, PageTypes, AssignTypes, VisuStatus, LogCode) {
    /*jshint white:false */
    'use strict';
    ...
    */
    ...
    function _activateVisuResponse(response, callbackInfo) {
        ...
        var visu = _visuModel.visus[callbackInfo.visuId],
            deferred = visu.activateDeferred,
            responseCode = (response !== undefined && response.status !== undefined) ? response.status.code : undefined;

        //console.always('_activateVisuResponse,visuId=' + callbackInfo.visuId + ',response:' + JSON.stringify(response));

        visu.active = (responseCode === 0);
        ...
        document.body.dispatchEvent(new CustomEvent(BreaseEvent.VISU_ACTIVATED, {
            detail: {
                visuId: callbackInfo.visuId, active: visu.active
            }
        }));
    }
