    ...
    p.createEvent = function (event, eventArgs) {
        if (this.settings.className && this.elem !== null) {
            if (this.events[event] !== undefined) {
                ...
                this.events[event] = new EventHandler(type + '.Event', this.elem.id, event, eventArgs, this.elem, this.settings.parentContentId);
                return this.events[event];
            }
        }
   ...
   */
	...
	*/
    ...
    p.suspend = function () {
        ...
        this.state = Enum.WidgetState.SUSPENDED;
    };

    /**
	* @method
	* Do not call wake directly but via uiController, otherwise arguments are missing
	* Example: brease.uiController.wake(widget.elem, true);
	*/
