...
define(function (require) {
    ...
    var BreaseEvent = require('brease/events/BreaseEvent'),
        Utils = require('brease/core/Utils'),
        config = require('brease/config'),
        EventHandler = require('brease/events/EventHandler');
    ...
    */
    ...
    function _loadTexts_responseHandler(response, callbackInfo) {
        var type = callbackInfo['type'];
        _texts = response;
        if (_isChange) {
            _isChange = false;
            document.body.dispatchEvent(new CustomEvent(BreaseEvent.LANGUAGE_CHANGED, { detail: { currentLanguage: _current.key } }));
            //window.performanceMonitor.profile('language_switch', { currentLanguage: _current.key, state: "finished" });

            var ev = new EventHandler("clientSystem.Event", "", "LanguageChanged", { Language: _current.key });
            ev.dispatch();

        } else {
            _finish(true, 'languages and texts loaded successfully');
            //if (type === 'system') {
            //    //window.performanceMonitor.profile('language_loadSystemTexts', { state: "finished" });
            //}
            //if (type === 'user') {
            //    //window.performanceMonitor.profile('language_loadUserTexts', { state: "finished" });
            //    //window.performanceMonitor.profile('language_initialLoad', { state: "finished" });
            //}
